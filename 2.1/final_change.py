...
if __name__ == "__main__":
    N_RUNS = 100
    # … your existing loop that fills peak_rabbits_all and final_foxes_all …
    # -------------------------------------------------------------------------
    # Baseline variability: compute mean and SD for two key metrics
    # -------------------------------------------------------------------------
    ...
    # Build a DataFrame for just these two metrics
    ...
    mean_final_r   = df_baseline['Final Rabbits'].mean()
    std_final_r    = df_baseline['Final Rabbits'].std(ddof=1)
    mean_final_f  = df_baseline['Final Foxes'].mean()
    std_final_f   = df_baseline['Final Foxes'].std(ddof=1)

    # 1) Print a small table in the console
    print("Baseline variability over {} runs:".format(N_RUNS))
    print("Metric          Mean     SD")
    print(f"Final Rabbits   {mean_final_r:7.2f}  {std_final_r:7.2f}")
    print(f"Final Foxes    {mean_final_f:7.2f}  {std_final_f:7.2f}")

    # 2) Bar chart with error bars
    metrics = ['Final Rabbits', 'Final Foxes']
    means   = [mean_final_r,    std_final_r]
    sds     = [mean_final_f,     std_final_f]

    plt.figure(figsize=(6,4))
    plt.bar(metrics, means, yerr=sds, capsize=5)
    plt.ylabel('Count')
    plt.title(f'Baseline ABM variability (N={N_RUNS})')
    plt.tight_layout()
    plt.savefig('baseline_bar_error.png', dpi=300)
    # plt.show()

    # 3) Histogram of peak-rabbit distribution
    plt.figure(figsize=(6,4))
    plt.hist(df_baseline['Final Rabbits'], bins=20, edgecolor='black')
    plt.xlabel('Final rabbit count')
    plt.ylabel('Frequency')
    plt.title('Distribution of Final rabbit counts (N={})'.format(N_RUNS))
    plt.tight_layout()
    plt.savefig('Final_rabbit_hist.png', dpi=300)
    # plt.show()
